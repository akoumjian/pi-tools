...
**Provides.** `shell_start`, `shell_status`, `shell_read`, `shell_cancel`; `/async:status`; and the interactive read-only `/async:view` selector/viewer for live or historical job output. Durable per-job logs and metadata remain under `.pi/async-shell/jobs/<jobId>/`. `shell_start` and completion notices point at `stdout_log`/`stderr_log` without embedding output samples; use `shell_read` tail mode for model inspection, or `/async:view` for provider-free human viewing. The viewer bounds reads to the existing 500-line/120 KB tail limits per stream, strips terminal control sequences for safe rendering, and closes without stopping jobs. The built-in Pi footer shows a theme-aware `shN` only while the exact current session has active user shell jobs; managed worker host processes are excluded, and the status clears at zero without adding history or provider context.
