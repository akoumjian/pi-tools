...
**Setup.** macOS, Node, Pi, a running Docker Desktop/Engine plus CLI, the pinned worker image documented in the full guide, a correctly routed central `bd` executable, and `config/worker-settings.json` with `defaultRoute`; `worker_review` additionally requires an exact `reviewRoute` (`provider/model:thinking`) and fails closed when the model or auth is unavailable. An optional different `reviewRateLimitFallbackRoute` must use the same provider and is attempted once in a fresh child only after a confirmed primary Anthropic HTTP 429 `rate_limit_error`; exact attempts remain visible. New workers use `defaultRoute` unless the caller supplies `route`; resume always retains the persisted implementation route. The parent remains responsible for grounding, review, integration, promotion, and task closure.
