...
test("shell_start schema exposes commands list and async follow-up guidance", () => {
  ...
  assert.match(shellStart.description, /Completion follow-ups are short result notices/);
...
test("buildNativeToolsSystemPrompt removes default Pi tool prose and injects read_many skill guidance", () => {
  ...
  assert.match(result, /Do not poll or wait, you will be notified/);
  assert.match(result, /Async-shell completion follow-ups are short result notices/);
