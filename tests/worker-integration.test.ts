...
test("provisions exact isolated integration inputs and enforces pristine analysis", async () => withTemp(async (root) => {
  ...
  const provisioned = provisionIntegrationRepository({ selection, repositoryPath, trustedStateRoot: path.join(state, "git") });
...
test("rejects context tampering and moved targets", async () => withTemp(async (root) => {
  ...
  assert.throws(() => provisionIntegrationRepository({ selection, repositoryPath: tamperedRepo, trustedStateRoot: path.join(root, "tampered-git") }), /immutable owner-only|hash mismatch/);
  ...
  assert.throws(() => assertPreparedTargetCurrent(selection.repository, path.join(root, "target-check")), /moved or became dirty/);
}));
...
test("accepts only a clean committed resolution and emits exact integration lineage", async () => withTemp(async (root) => {
  ...
  const integration: WorkerIntegrationRecord = {
    ...
    preparedArtifactFile: path.join(selection.preparedDirectory, selection.repository.artifact.file), workspaceRepo: "repos/integration", contextFile: context.file, workspaceContextFile: context.workspaceFile, contextSha256: context.sha256,
  ...
  validateIntegrationHandoff(handoff, integration, workspace, path.join(state, "git"));
  const inventory = deriveRepositoryInventory({
    ...
    integrationLineage: { workspaceRepo: integration.workspaceRepo, lineage: { kind: "integration_resolution", preparedId: integration.preparedId, manifestSha256: integration.manifestSha256, sourceCandidateIds: integration.sourceCandidateIds, contextSha256: integration.contextSha256, decisionsSha256: integration.decisionsSha256!, analysisRunId: integration.analysisRunId, resolutionRunId: integration.resolutionRunId!, targetExpectedCommit: integration.targetExpectedCommit, targetExpectedTree: integration.targetExpectedTree } }
  });
  ...
  assert.throws(() => validateIntegrationHandoff(handoff, integration, workspace, path.join(state, "git")), /input refs changed/);
  assert.throws(() => validateIntegrationHandoff({ ...handoff, repositories: [...handoff.repositories, { workspaceRepo: "repos/extra", purpose: "unexpected" }] }, integration, workspace, path.join(state, "git")), /report exactly/);
}));
