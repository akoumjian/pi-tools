...
5. [worker](extensions/worker.md) — durable exact-session Pi workers with private workspaces and typed handoffs.
6. [completion-notifications](extensions/completion-notifications.md) — opt-in privacy-safe terminal notification after a settled long run.
7. [native-tools](extensions/native-tools.md) — batch-native `read_many`, `search_many`, `write_many`, `edit_many` and strict-replacement.
8. [manual-retry](extensions/manual-retry.md) — provider-safe `/retry` that resends the current context after any failure or cancellation, plus bounded automatic retry of outage-class errors.
9. [context-export](extensions/context-export.md) — provider-free `/context:copy` for an active, compaction-aware clipboard snapshot.
10. [compacter](extensions/compacter.md) — robust chunked replacement for Pi compaction with `/compacter` controls.
11. [mutation-review](extensions/mutation-review.md) — file-mutation reuse reviewer with cached re-apply via `apply_reviewed_mutation`.
12. [searxng-search](extensions/searxng-search.md) — `searxng_search` against a configured self-hosted SearXNG instance.
