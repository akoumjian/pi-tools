...
6. [manual-retry](extensions/manual-retry.md) — provider-safe `/retry` for a settled transient failure without duplicate user text.
7. [context-export](extensions/context-export.md) — provider-free `/context:copy` for an active, compaction-aware clipboard snapshot.
8. [compacter](extensions/compacter.md) — robust chunked replacement for Pi compaction with `/compacter` controls.
9. [mutation-review](extensions/mutation-review.md) — file-mutation reuse reviewer with cached re-apply via `apply_reviewed_mutation`.
10. [searxng-search](extensions/searxng-search.md) — `searxng_search` against a configured self-hosted SearXNG instance.
11. [web-fetch](extensions/web-fetch.md) — `web_fetch_many` for safe HTTP(S) fetch + cache + readability extraction.
12. [file-open](extensions/file-open.md) — `/file:open` picker for recent transcript file references.
13. [theme-preview](extensions/theme-preview.md) — `/themes:preview` live theme showcase.
14. [review-subagent](extensions/review-subagent.md) — `/review` tool-using review subagent workflow.
15. [tool-display](extensions/tool-display.md) — opt-in display wrapper for `document_parse`.
16. [orchestrator](extensions/orchestrator.md) — bounded model-routed readers/planners/writers with worktree review and reconciliation.
