...
The built-in Pi footer uses the extension status surface for one compact theme-aware `shN` count while the exact current session has active user `shell_start` jobs. It derives the count from the canonical attached job registry on start and terminal transitions, excludes managed worker host processes, clears at zero and session shutdown, and never adds a transcript message, provider request, assistant turn, polling timer, or second job registry. The canonical in-process map and its listener set live in one version-checked `globalThis` holder, so an uncached extension `/reload` sees pre-reload live jobs and old lifecycle owners notify the new TUI listener; an incompatible holder version fails loudly. The old extension shutdown removes only its own listener. Pi 0.84.4 exposes no extension theme-change event, so the status re-renders from the current theme on session start/reload, each shell lifecycle event, and the next submitted input after a theme selection. Its priority key sorts after `wN` but before verbose extension statuses; Pi may still truncate statuses at an extremely narrow terminal width.
...
The activity event guarantee is deliberately in-process and single-writer. Two Pi processes opening the same parent session do not share live maps or listener events, so duplicate concurrent opens are outside the exact footer guarantee. The durable shell tools retain their normal metadata and process validation, but this compact status adds no filesystem watcher, polling loop, or cross-process synchronization.
