...
- `worker_fold_resolve({ kind: "start" | "resume", ... })`
- `/worker:list [--all]`
...
Resolution is strictly two phase in one worker identity, Pi session, workspace, and fixed provider route. The analysis run receives read-only context/evidence and exact target/candidate identities but must leave HEAD/tree, tracked and untracked status (including ignored files), index, refs, repository config, and reachable object identity equal to the host snapshot. Its only accepted handoff states are `checkpoint` and `needs_input`. After the parent observes that exact analysis result, a `kind:"resume"` call requires an immutable nonempty settled-decisions addendum; generic `worker_run` resume is refused. The host revalidates context, prepared state, target state, and the pristine analysis snapshot before writing the decisions artifacts and permitting repository mutation. Resolution must hand off only `ready_for_review` or `assignment_complete`, report exactly the integration repository, retain the prepared target as an ancestor, and produce a committed changed tree. Target movement, artifact/context/decision tampering, repository-policy failures, wrong handoff states, or phase mixing fail closed at admission and again at trusted settlement.

A successful resolution handoff is inventoried as a new ordinary candidate; it does not modify or replace the prepared manifest and does not promote anything. Its inventory lineage binds the prepared ID/manifest, source candidate IDs, context and decisions hashes, analysis and resolution run IDs, and exact prepared target commit/tree. Review, validation, fold preparation of that new candidate, local promotion, and publication remain separate parent-owned operations.
