...
Integration context envelopes are bounded to 128 KiB; evidence is limited to 32 regular single-link files, 2 MiB each and 8 MiB aggregate; the trusted immutable index copy is limited to 64 MiB; prepared bundles are limited to 256 MiB and hashed with bounded streaming reads; non-object Git metadata is limited to 20,000 entries and 32 MiB. Repository discovery is limited to 32 repositories, depth 12, and 20,000 traversed directories. Ordinary files do not consume traversal capacity because they cannot contain a nested repository; every nonsymlink directory still does, so nested-repository detection is not silently skipped. Exceeding a directory/depth/repository bound produces the corresponding explicit incomplete scan limitation and integration settlement fails closed.
