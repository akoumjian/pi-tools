...
2. A new worker is deferred until `turn_end`, then forks the exact completed parent session with `SessionManager.forkFrom(...)`.
...
- Multi-repository folding and promotion are separate later capabilities; worker handoffs do not authorize candidate integration.
