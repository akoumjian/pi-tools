...
`worker_fold_prepare` is the lean `RepositoryChangeSet` boundary. Each of at most 16 mappings names one durable `candidateId`, one absolute existing local target repository below the accepted local target root (`~/Code` by default), one fully qualified existing `refs/heads/...` target, a purpose, `merge` or `squash`, and optional candidate dependencies. The host resolves candidates only from hash-validated inventories owned by the exact parent session, rejects missing/duplicate/cyclic selection and more than one candidate per physical target, then revalidates candidate base/head/tree/dirty/policy facts plus target ref/tree/cleanliness/policy and linked-worktree state. Dirty candidate state remains an explicit inspection fact and does not by itself invalidate an exact committed delta; dirty targets fail closed.

Preparation uses the same absolute, shell-free, callback-disabled, bounded Git boundary as inventory. It creates bounded bundles and clones those bundles into an owner-private staging directory, computes clean merge trees and deterministic merge/squash commits, and materializes disposable views. It atomically publishes an immutable versioned manifest under `~/.local/share/agent/worker-folds/<prepared-id>/` with its SHA-256, dependency order, exact candidate/target/input/output OIDs, relative artifact paths and hashes, repository-level overlap counts without changed-path enumeration, explicit discrepancy collections, and bounded resolution cases. A merge conflict leaves the view at the exact expected target and emits `resolution_required` without inventing a desired commit. Target refs, indexes, worktrees, files, remotes, and object stores are checked before and after and are never written. The durable reader revalidates manifest identity/hash, bundle hashes, and clean view HEAD/tree state after restart. This tool does not review, validate, resolve conflicts, promote, fetch, push, publish, or call a model.
