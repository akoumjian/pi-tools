...
9. When explicitly requested later, fold preparation resolves selected candidates from those exact inventories and writes only external immutable prepared state; authoritative targets remain unchanged.
...
- This slice derives durable repository-level candidates and implements nonauthoritative `worker_fold_prepare`. Conflict resolution, review/validation binding, and local exact-ref promotion remain separate later capabilities; neither a handoff, candidate, nor prepared artifact authorizes integration.
